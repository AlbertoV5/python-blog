...
:PROPERTIES:
:CUSTOM_ID: globbing-image-files
:END:

This is an exercise on using globbing[fn:1] to find image files in a directory. We are going to use the =pathlib=[fn:2] Python module which provides the =glob=[fn:4] in the =Path= class.
...
#+NAME: get_imgs
#+begin_src python :tangle ./globbing/imgs.py
<<imports>>
...
Note that we are placing the results in a parent directory different to the one we use for globbing as we are searching recursively by default. This is because we don't want to get our results as inputs the second time we run the script.
...
The benefit of using globbing is that we don't have to use Python to check that each file suffix matches a list of file formats so we can scale our process more easily.
