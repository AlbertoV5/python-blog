...
Before creating the files in the test directory, we are going to add a few children directories so we can test recursive search too. We are going to create 2 'tuneable' variables, =depth= and =n=, which will effectively bring the total amount of directories to =depth ** n=. Then we are going to create the directories somewhat recursively while adding each one to a =directories= list.
